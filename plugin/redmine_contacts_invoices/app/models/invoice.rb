...
class Invoice < ActiveRecord::Base
  ...
  def lines_from_issues(issues_ids, grouping, options={})
    invoice_project = self.project || @project || options[:project]
    self.lines ||= []
    case grouping.to_i
    when 1
      total_time = TimeEntry.sum(:hours, :conditions => {:issue_id => issues_ids}, :group => :issue_id)
      total_time.first
      total_time.each do |k, v|
        issue = Issue.find_by_id(k.to_i)
        self.lines << InvoiceLine.new(:description => issue.subject,
                                      :units => l(:label_invoice_hours),
                                      :quantity => "%.2f" % total_time[k].to_f)
      end
    when 2
      total_time = TimeEntry.sum(:hours, :conditions => {:issue_id => issues_ids}, :group => :user_id)
      total_time.first
      total_time.each do |k, v|
        scope = Issue.scoped(:include => :time_entries,  :conditions => {:id => issues_ids})
        scope = scope.scoped(:conditions => ["#{TimeEntry.table_name}.user_id = ?", k])
        issues = scope.all
        user = User.find(k)
        self.lines << InvoiceLine.new(:description => user.name.humanize +
                                        "\n" +
                                        issues.map{|i| " - #{i.subject} (#{l_hours(i.time_entries.where(:user_id => k).sum(:hours))})"}.join("\n"),
                                      :units => l(:label_invoice_hours),
                                      :quantity => "%.2f" % total_time[k].to_f,
                                      :price => RedmineInvoices.default_user_rate(user, invoice_project))
      end
    when 3
      total_time = TimeEntry.sum(:hours, :conditions => {:issue_id => issues_ids})
      scope = Issue.scoped(:include => :time_entries, :conditions => {:id => issues_ids})
    ...
    when 5
      scope = TimeEntry.scoped(:include => [{:issue => :tracker}, :activity, :user], :conditions => {:issue_id => issues_ids})
      ...
      total_time = TimeEntry.sum(:hours, :conditions => {:issue_id => issues_ids}, :group => :activity_id)
      total_time.first
      total_time.each do |k, v|
        ...
        self.lines << InvoiceLine.new(:description => Enumeration.find(k).name.humanize +
                                            "\n" +
                                            issues.map{|i| " - #{i.subject} (#{l_hours(i.time_entries.where(:activity_id => k).sum(:hours))})"}.join("\n"),
                                          :units => l(:label_invoice_hours),
                                          :quantity => "%.2f" % total_time[k].to_f)
      end
    end
  ...
  def lines_from_time_entries(time_entries_ids, grouping, options={})
    invoice_project = self.project || @project || options[:project]
    self.lines ||= []
    case grouping.to_i
    when 1 # By issues
      total_time = TimeEntry.sum(:hours, :conditions => {:id => time_entries_ids}, :group => :issue_id)
      total_time.first
      total_time.each do |k, v|
        issue = Issue.find_by_id(k.to_i)
        self.lines << InvoiceLine.new(:description => issue.subject,
    ...
    when 2 # by users
      total_time = TimeEntry.sum(:hours, :conditions => {:id => time_entries_ids}, :group => :user_id)
      total_time.first
      total_time.each do |k, v|
        scope = Issue.scoped(:include => :time_entries,  :conditions => {:time_entries => {:id => time_entries_ids}})
        scope = scope.scoped(:conditions => ["#{TimeEntry.table_name}.user_id = ?", k])
        issues = scope.all
        user = User.find(k)
        self.lines << InvoiceLine.new(:description => user.name.humanize +
                                        "\n" +
                                        issues.map{|i| " - #{i.subject} (#{l_hours(i.time_entries.where(:id => time_entries_ids).where(:user_id => k).sum(:hours))})"}.join("\n"),
                                      :units => l(:label_invoice_hours),
                                      :quantity => "%.2f" % total_time[k].to_f,
                                      :price => RedmineInvoices.default_user_rate(user, invoice_project))
      end
    when 3 # in one line
      total_time = TimeEntry.sum(:hours, :conditions => {:id => time_entries_ids})
      scope = Issue.scoped(:include => :time_entries, :conditions => {:time_entries => {:id => time_entries_ids}})
      scope = scope.scoped(:conditions => ["#{TimeEntry.table_name}.hours > 0"])
      issues = scope.all
      self.lines << InvoiceLine.new(:description => issues.map{|i| " - #{i.subject} (#{l_hours(i.time_entries.where(:id => time_entries_ids).sum(:hours))})"}.join("\n"),
                                    :units => l(:label_invoice_hours),
                                    :quantity => total_time)
    when 5 # by time entries
      scope = TimeEntry.scoped(:include => [{:issue => :tracker}, :activity, :user], :conditions => {:id => time_entries_ids})
      time_entries = scope.all
      time_entries.each do |time_entry|
        issue = time_entry.issue
        line_description = "#{time_entry.activity.name} - #{issue.tracker.name} ##{issue.id}: #{issue.subject} #{'- ' + time_entry.comments + ' ' unless time_entry.comments.blank?}"
      ...
      total_time = TimeEntry.sum(:hours, :conditions => {:id => time_entries_ids}, :group => :activity_id)
      total_time.first
      total_time.each do |k, v|
        ...
        self.lines << InvoiceLine.new(:description => Enumeration.find(k).name.humanize +
                                            "\n" +
                                            issues.map{|i| " - #{i.subject} (#{l_hours(i.time_entries.where(:id => time_entries_ids).where(:activity_id => k).sum(:hours))})"}.join("\n"),
                                          :units => l(:label_invoice_hours),
                                          :quantity => "%.2f" % total_time[k].to_f)
      end
    end
