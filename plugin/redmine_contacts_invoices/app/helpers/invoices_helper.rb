...
module InvoicesHelper
  ...
  def invoice_to_pdf(invoice)
    if InvoicesSettings.is_custom_template?(@project) && !InvoicesSettings.custom_template(@project).blank?
      invoice_to_pdf_wicked_pdf(invoice)
    else
  ...
  def invoice_to_pdf_wicked_pdf(invoice)
    ...
    content = render_to_string(:text => liquidize_invoice(invoice), :layout => 'invoices.pdf')
  ...
  def liquidize_invoice(invoice)
    content = InvoicesSettings.custom_template(@project)
    assigns = {}
    ...
    content = Liquid::Template.parse(content).render(Liquid::Context.new({}, assigns, registers)).html_safe
  end
