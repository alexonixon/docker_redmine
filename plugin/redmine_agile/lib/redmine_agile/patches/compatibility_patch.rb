...
class AgileQuery < Query
  ...
  def swimlanes
    ...
    lane_ids = self.issues.group(self.group_by_column.groupable).count.keys
    lanes = Issue.reflect_on_association(self.group_by_column.name).klass.where(:id => lane_ids)
